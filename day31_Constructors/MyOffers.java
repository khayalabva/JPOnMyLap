...
public class MyOffers {
    public static void main(String[] args) {
       ...
       /* ArrayList<Offer> fullTimeJobs = new ArrayList<>();
        for (Offer offer : myOffers){
            if(offer.ifFullTime){
                fullTimeJobs.add(offer);
            }
        }
        System.out.println(fullTimeJobs);

        */

        ArrayList<Offer> fullTimeOffers = new ArrayList<>( Arrays.asList( myOffers )  );
        fullTimeOffers.removeIf( p -> ! p.ifFullTime ); // removes the offer if the offer is NOT fulltime
